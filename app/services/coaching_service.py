...
class CoachingService:
        ...
        try:
            ...
            try:
                ...
                if len(questions_data) != 5:
                    raise ValueError(f"예상된 5개 질문이 아닌 {len(questions_data)}개 질문이 생성됨")
    ...
    def _generate_fallback_questions(self, resume_data: ResumePayload) -> List[InterviewQuestion]:
    ...
    def _generate_fallback_learning_path(self, resume_data: ResumePayload) -> LearningPath:
        ...
        steps = [
            LearningStep(
                ...
                resources=[f"{skill} 고급 가이드" for skill in primary_skills[:2]] + ["성능 최적화 패턴"]
            ),
            LearningStep(
                title="시스템 설계 역량 개발",
                description="대규모 시스템 아키텍처 설계 경험을 쌓기 위한 사이드 프로젝트 진행",
                resources=["System Design Interview", "마이크로서비스 아키텍처", "분산 시스템 설계"]
            ),
            LearningStep(
                title="기술 리더십 함양",
                description="코드 리뷰 문화 구축, 기술 문서 작성, 팀 내 지식 공유 활동 참여",
                resources=["Effective Code Review", "기술 블로깅", "컨퍼런스 발표 준비"]
            )
        ]
