...
class InterviewQuestion(BaseModel):
    ...
    category: str = Field(..., description="질문 유형 (예: Technical Deep-Dive, Behavioral, Situational)")
...
class CoachingResult(BaseModel):
    ...
    session_id: UUID = Field(default_factory=uuid4, description="코칭 세션의 고유 식별자")
    interview_questions: List[InterviewQuestion] = Field(
        ..., 
        description="생성된 면접 질문들 (5개)",
        min_items=5,
        max_items=5
    )
